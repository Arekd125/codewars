...
//  that I really recommend to any math enthusiast and for showing me this mathematical curiosity
...
std::vector<int> tribonacci(std::vector<int> signature, int n)
{
    if (n >= 3)
    {
        for (int i = 0; i < n - 3; i++)
        {
            int a = signature.at(i);
            int b = signature.at(i + 1);
            int c = signature.at(i + 2);

            signature.push_back(a + b + c);
        }
